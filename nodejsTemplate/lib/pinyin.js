...
var pinyin = module.exports = function (dict) {
   ...
   *                        否则，返回以 sp 分隔的字符串。
   */
  return function (hans, lowercase, single, sp) {
    var len = hans.length;
    if (len == 0) {
      return single ? "" : [];
    }
    if (len == 1) {
      ...
      if (single) {
        if(lowercase)
          return y && y[0] ? trans2LowerCase(y[0]) : hans;
        else
          return y && y[0] ? y[0] : hans;
      }
      if(lowercase)
        return trans2LowerCase(y) || [hans];
      else
        return y || [hans];
    } else {
      var py = [];
      ...
      if (single) {
        if(lowercase)
          return sp == null ? trans2LowerCase(py) :
            trans2LowerCase(py.join(sp || ""));
        else
      ...
      if(lowercase)
        return sp == null ? trans2LowerCase(pys)
          : trans2LowerCase(pys.join(sp || ""));
      else
